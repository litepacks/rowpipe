...
export async function limitCommand(
  ...
  const effectiveLimit = Math.max(0, Number(count) || 10);
  ...
  const pipeline = createPipeline(reader, { batchSize: effectiveBatchSize, maxRows: effectiveLimit });
  pipeline.pipe(limitRows(effectiveLimit));
